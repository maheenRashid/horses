...
try:
    path_horse = sys.argv[1];
    out_folder = sys.argv[2]; 
except IndexError:
    print "Usage: python valid_not_valid.py <path-to-txt-horse> <out_folder>"
    sys.exit(1)

lines = util.readLinesFromFile(path_horse);
lines = [line_curr.split(' ') for line_curr in lines];

for idx,line_curr in lines:
    img_horse_path=line_curr[0];
    img_human_path=line_curr[1];
    
    file_new=os.path.split(img_horse_path)[1];
    file_new=file_new[:file_new.rindex('.')];
    out_file=os.path.join(out_folder,file_new+'.txt');

    img_horse=Image.open(img_horse_path);
    img_human=Image.open(img_human_path);

    print img_horse_path;
    print img_human_path;
    print img_horse;
    print img_human;
...
# monitor_w = 1680.0-150.0
# monitor_h = 1050.0-300
# min_w = 400
# min_h = 400
# N_points = 7
# point_now = 0
...
#     if size[0] > monitor_w or size[1] > monitor_h:
#         print 'resized'
#         resized = True
#         resize_multiple = max([math.ceil(size[0]/monitor_w), math.ceil(size[1]/monitor_h)])
...
#         print 'resized'
#         resized = True
#         if min(size[0], size[1] < 300):
#             resize_multiple = 0.25
#         else:
#             resize_multiple = 0.5
#         #resize_multiple = min([(size[0]/(monitor_w/2)), (size[1]/(monitor_h/2))])
#         print 'resize multiple: ', resize_multiple
#         img_size[0] = int(size[0] / resize_multiple)
#         img_size[1] = int(size[1] / resize_multiple)
#         img = img.resize(tuple(img_size), Image.ANTIALIAS)
...
#     def clearPoint(num):
#         print 'clear point: ', annotationInfo[num]
#         global rect
#         global bbox
#         if rect[num] != None:
#             canvas.delete(rect[num])
