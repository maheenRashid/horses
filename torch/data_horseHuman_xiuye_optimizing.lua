...
do  
        ...
        if num_problem>0 then
            ...
            for img_path_curr,img_path_curr_index in pairs(images_index_old) do
                if not problem_idx[img_path_curr_index] then
                    ...
                    -- lines_human_new[i]=self.lines_human[img_path_curr_index];
                end
            end
            ...
            -- self.lines_human=lines_human_new;
        end
        
        images_all:mul(255);
        for i=1,images_all:size(2) do
            images_all[{{},i,{},{}}]:csub(self.params.mean[i])
        end
    ...
    function data:getBatchPoints(debug_flag)
        ...
        for idx_curr=1,self.training_set_horse.label:size(1) do
            ...
            -- print (label_curr_pos_human:size(),label_curr_pos_human:size(1)>2)
        ...
        local data;
        if #data_idx==self.training_set_horse.data:size(1) then
            -- print 'equal';
            data=self.training_set_horse.data;
        else
            data=torch.zeros(#data_idx,self.training_set_horse.data:size(2),self.training_set_horse.data:size(3),
    ...
    function data:addTrainingDataNoIm(training_set_horse,training_set_human,batch_size,lines_horse,lines_human,start_idx_horse,params)
        ...
        while curr_idx<= batch_size do
            ...
            if self.images_index[img_path_horse] then
                -- t=os.clock()
                local img_horse=self.images_all[self.images_index[img_path_horse]]:clone();
            ...
            -- if status_img_horse then
                ...
                training_set_horse.data[curr_idx]=img_horse:int():clone();
                
                training_set_horse.label[curr_idx]=label_horse:clone();
                training_set_human.label[curr_idx]=label_human:clone();
